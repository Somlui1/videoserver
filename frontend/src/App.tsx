    ...
    <aside className="w-80 bg-surface-container-lowest border-r border-outline-variant/30 flex flex-col h-full shadow-2xl z-20">
      <div className="p-10 border-b border-outline-variant/30">
        <div className="flex items-center gap-4 mb-2">
          ...
          <h1 className="text-[28px] font-black text-primary leading-none tracking-tighter font-bai uppercase">AAPICO</h1>
        </div>
        <p className="text-[10px] text-outline font-black uppercase tracking-[0.3em] ml-1">Video Portal v2.4</p>
      </div>
...
const Layout = ({ children, logs, addLog, currentPath }: { children: React.ReactNode, logs: LogEntry[], addLog: (msg: string) => void, currentPath: string }) => {
...
const LoginPage = ({ addLog }: { addLog: (msg: string) => void }) => {
  ...
  return (
    <main className="flex min-h-screen w-full flex-col md:flex-row bg-surface selection:bg-secondary-container selection:text-on-secondary-container font-bai">
      {/* Left Column: The Monolith Brand Anchor */}
      <section className="relative w-full md:w-1/2 bg-primary-container flex flex-col justify-between p-12 lg:p-20 overflow-hidden">
        ...
        <div 
          ...
          style={{ 
            backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuAGFBQR0SwdHCCBccrAdG_je3c8bMds4JtA-N-WSF1fgAk04PTHaiM-SpAZICTnDtnBXlwO5AB3yvyw45qhZSgk8gIutSAhhfOJOBJotFz6s0UeiXPm_JDZANszPG6sOI3iyCj83BPfA6zGO-cC2IBTmoxx83g-Dq9oNMHm-pv3z7ZDz2o5__L5ZfR0ImMDRYbTRH-1T9lUCC-xnhQZPRPyCN7ddkSTTHYz58Uwi7VDj4O1COQfeBaSmANZYj9zvYsICSrYFpz5Pm0r')", 
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-white text-2xl font-bold tracking-tighter leading-none">AAPICO</span>
              <span className="text-on-primary-container text-xs font-medium tracking-[0.2em] uppercase">Enterprise Secure</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 mt-24">
          <h1 className="text-white text-5xl lg:text-7xl font-bold leading-[1.1] tracking-tighter max-w-lg mb-8">
            Steadiness Meets <br/>
            <span className="text-secondary-fixed">Innovation.</span>
          </h1>
          <p className="text-on-primary-container text-lg max-w-md font-light leading-relaxed">
            Accessing the AAPICO Video Management Portal via Federated Identity. Ensuring global corporate stability through precise digital control.
          </p>
        </div>
      ...
      {/* Right Column: Focused Interaction Canvas */}
      <section className="w-full md:w-1/2 bg-white flex items-center justify-center p-8 lg:p-24">
        <div className="w-full max-w-md flex flex-col gap-12">
          {/* Headline Section */}
          <div className="flex flex-col gap-4">
            <div className="h-1 w-16 bg-primary"></div>
            <h2 className="text-on-surface text-[32px] font-bold leading-tight tracking-tight">
              Login with LDAP Credentials
            </h2>
            <p className="text-on-surface-variant text-sm">
              Please enter your corporate account details to proceed to the management dashboard.
            </p>
          </div>
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-[48px] font-black text-primary leading-none tracking-tight font-bai uppercase">Upload Video</h1>
          <p className="text-[16px] text-on-surface-variant mt-2 max-w-2xl font-bai">Push new educational assets to the enterprise CDN and start the transcoding pipeline.</p>
        </div>
        <button 
...
const LibraryPage = ({ addLog }: { addLog: (msg: string) => void }) => {
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-[48px] font-black text-primary leading-none tracking-tight font-bai uppercase">Video Library</h1>
          <p className="text-[16px] text-on-surface-variant mt-2 max-w-2xl font-bai">Manage and organize enterprise educational content across all corporate divisions.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        {videos.map((v) => (
          <div key={v.id} className="group bg-surface-container-lowest flex flex-col transition-all hover:translate-y-[-4px] border border-outline-variant/10">
            <div className="relative aspect-video bg-tertiary overflow-hidden">
              <img 
                className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500" 
                src={v.thumbnailUrl || VideoService.getThumbnailUrl(v.id)}
                alt={v.title}
                onError={(e) => {
              ...
              <div className="absolute top-4 left-4">
                <span className={cn(
                  "text-[10px] font-black px-2 py-1 uppercase tracking-tighter",
                  v.status === 'ready' ? "bg-secondary text-white" : "bg-primary-container text-white"
                )}>
                  {v.status}
                </span>
              </div>
              {v.status === 'ready' && (
                <div className="absolute bottom-4 right-4 bg-tertiary/80 backdrop-blur-md px-2 py-1 text-[10px] text-white font-mono">04:22</div>
              )}
              {v.status === 'processing' && (
                <div className="absolute bottom-0 left-0 h-1 bg-secondary w-1/3"></div>
              )}
            </div>
            
            <div className="p-6 flex flex-col flex-1">
              <span className="text-[10px] font-mono text-outline mb-1 uppercase tracking-widest">ID: {v.id.substring(0, 8).toUpperCase()}</span>
              ...
              <div className="mt-auto flex flex-col gap-4">
                <div className="flex items-center justify-between text-[14px] text-on-surface-variant">
                  <span className="flex items-center gap-1">
                    <span className="material-symbols-outlined text-sm">calendar_month</span> Oct 12, 2023
                  </span>
                  <span className="flex items-center gap-1">
                ...
                <div className="grid grid-cols-3 gap-1 pt-4 border-t border-surface-container">
                  <button className="flex items-center justify-center py-2 text-primary-container hover:bg-surface-container-high transition-colors">
...
const AdminJobsPage = ({ addLog }: { addLog: (msg: string) => void }) => {
  const [videos, setVideos] = useState<Video[]>([]);
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-[48px] font-black text-primary leading-none tracking-tight font-bai uppercase">Admin Console</h1>
          <p className="text-[16px] text-on-surface-variant mt-2 max-w-2xl font-bai">Real-time processing status and server telemetry for the enterprise transcoding cluster.</p>
        </div>
        <button 
          onClick={fetchData}
          disabled={loading}
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {videos.map((video) => (
                <tr key={video.id} className="hover:bg-surface-container-lowest/50 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <span className="px-3 py-1 bg-surface-container-high text-on-surface-variant text-[10px] font-bold uppercase tracking-widest sharp-edge border border-outline-variant/20">{video.quality || '1080p'} / 720p / 480p</span>
                  </td>
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route
          ...
          element={
            <ProtectedRoute>
              <AppRoutes logs={logs} addLog={addLog} />
            </ProtectedRoute>
          }
        />
      </Routes>
...
function AppRoutes({ logs, addLog }: { logs: LogEntry[], addLog: (msg: string) => void }) {
  ...
  return (
    <Layout logs={logs} addLog={addLog} currentPath={currentPath}>
      <Routes>
        <Route path="/upload" element={<UploadPage addLog={addLog} />} />
        <Route path="/library" element={<LibraryPage addLog={addLog} />} />
        <Route path="/admin" element={<AdminJobsPage addLog={addLog} />} />
        <Route path="/settings" element={<div className="bg-white p-6 rounded-lg shadow-md border border-gray-100"><h2 className="text-xl font-bold text-[#2c3e50]">Settings</h2><p className="text-gray-500 mt-4">System configuration and user preferences.</p></div>} />
